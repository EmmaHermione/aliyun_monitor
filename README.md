...
- 🌍 **Cloudflare DDNS**：可为每台 ECS 单独配置 DNS 记录，实例处于运行窗口内且 Running 时自动同步公网 IP。
...
本项目支持给每台 ECS 实例单独配置 Cloudflare DDNS。第一版 DDNS 不做全局主备抢占，也不会在实例停机时清空 DNS；自动巡检仅在实例处于自己的运行窗口内且状态为 `Running` 时，同步该实例配置的 DNS 记录。通过 Telegram Bot 手动开机时，确认实例进入 `Running` 后也会立即同步该实例的 DNS 记录。

同步时机：

* 自动巡检发现实例需要恢复运行，并确认已进入 `Running` 后。
* 实例已在运行窗口内且保持 `Running` 时，按冷却周期检查公网 IP，避免每 5 分钟重复调用 Cloudflare。
* Telegram Bot 手动开机后，确认实例进入 `Running` 时。

同一个 DNS 记录被多台实例共用时，巡检会优先处理当前处于运行窗口内的实例。只有接班实例已进入 `Running`，并且 DDNS 同步成功或无需更新后，窗口外的旧实例才会按计划节省停机。为避免 DNS 缓存导致切换期间不可用，接班实例就绪后旧实例还会额外延迟 5 分钟（`DDNS_HANDOFF_LINGER_SECONDS = 300`）再节省停机，确保全球 DNS 递归解析器中的旧缓存过期后再下线。DDNS 换班保护只是短暂缓冲，不高于流量熔断：旧实例暂缓与延迟期间仍会检查流量，超出阈值会立即节省停机；如果接班实例持续失败，旧实例最多暂缓 15 分钟，之后恢复遵循定时窗口。
...
如果多台实例配置同一个域名，建议运行窗口不要重叠，例如 `00:00-12:00` 与 `12:00-00:00`。窗口重叠时，多台 Running 实例仍可能先后同步同一条 DNS 记录，最后一次同步成功的实例会覆盖记录。
