...
同一个 DNS 记录被多台实例共用时，巡检会优先处理当前处于运行窗口内的实例。只有接班实例已进入 `Running`，并且 DDNS 同步成功或无需更新后，窗口外的旧实例才会按计划节省停机。DDNS 换班保护只是短暂缓冲，不高于流量熔断：旧实例暂缓期间仍会检查流量，超出阈值会立即节省停机；如果接班实例持续失败，旧实例最多暂缓 15 分钟，之后恢复遵循定时窗口。
