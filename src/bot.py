...
START_WAIT_INTERVAL_SECONDS = 10
...
def wait_until_running(user, timeout=START_WAIT_TIMEOUT_SECONDS):
    waited = 0
    while waited < timeout:
        time.sleep(START_WAIT_INTERVAL_SECONDS)
        waited += START_WAIT_INTERVAL_SECONDS
        status, inst = get_status(user)
        if status == "Running":
            return status, inst
        if status in ("NotFound", "Stopped"):
            return status, inst
...
def clear_schedule(config, chat_id, key):
    ...
    save_config(config)
    send_message(config, chat_id, f"✅ *[定时计划]*\n\n机器: {user_label(user)}\n动作: 已删除定时计划，恢复全天运行", parse_mode="Markdown")
